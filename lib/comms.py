...
class StealthConn(object):
    ...
    def initiate_session(self):
        ...
        if self.server or self.client:
            ...
            self.key = self.csprng.pseudo_random_data(32)

            # Set up a SHA384 HMAC with a 384-bit key generated by the CSPRNG
            self.hmac = HMAC.new(self.csprng.pseudo_random_data(SHA384.digest_size), None, SHA384.new())
    ...
    def recv(self):
        ...
        if self.key:
            ...
            # Retrieve the given HMAC            
            given_hmac = encrypted_data[-96:]

            # Calculate the HMAC of the encrypted data
            self.hmac.update(encrypted_data[AES.block_size:-96])
            message_hmac = self.hmac.hexdigest().encode()
            ...
            # Decrypt the data
            cipher = AES.new(self.key, AES.MODE_CFB, iv)
            data = cipher.decrypt(encrypted_data[AES.block_size:-96])
            if self.verbose:
